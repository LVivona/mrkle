...
/// Joint set of errors that could be found within the construction, the proof, and obtaining of nodes.
#[derive(Debug, thiserror::Error)]
pub enum MrkleError {
    /// A tree could not be constructed. due to sub errors.
    #[error("{0}")]
    ConstructionError(#[from] TreeBuilderError),
...
/// Merkle Proof Errors found when verifying or constructing a [`MrkleProof`].
#[derive(Debug, thiserror::Error)]
pub enum ProofError {
    ...
    /// Proofs are only generated by leaves.
    ///
    ...
    /// Internal tree error, when generating a new proofs.
    ///
